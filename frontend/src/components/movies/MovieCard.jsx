...
export default function MovieCard({ movie, groupId, onClick, onRefresh, style }) {
  ...
  const handleWatchedToggle = async (e) => {
    ...
    try {
      const { data } = await api.post('/watched/toggle', {
        movieId: movie._id,
        groupId,
      });
    ...
    >
      ...
      <div className="aspect-[2/3] rounded-xl overflow-hidden bg-cinema-card border border-cinema-border relative">
        ...
        <img
          ...
          onError={(e) => {
            e.target.src = getPosterPlaceholder();
          }}
        />
        ...
        <button
          ...
          className={`absolute top-12 right-2 w-8 h-8 rounded-full flex items-center justify-center transition-all duration-200 z-10
            ${
              isWatchLater
        ...
        {movie.genres?.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-1.5">
            ...
            {movie.genres.slice(0, 2).map((g) => (
              <span
                key={g}
                className="text-xs text-cinema-subtle bg-cinema-muted px-1.5 py-0.5 rounded"
              >
                {g}
              </span>
            ))}
